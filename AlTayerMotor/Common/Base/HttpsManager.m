...
{
    if (!_policy) {
        _policy = [AFSecurityPolicy policyWithPinningMode:AFSSLPinningModeCertificate];
        [_policy setValidatesDomainName:NO];
        [_policy setAllowInvalidCertificates:YES];
        NSString *sslPublicKeyPath = [[NSBundle mainBundle] pathForResource:@"atm" ofType:@"cer"];
